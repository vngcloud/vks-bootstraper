...
_kubeadmin_ignition_config_path = "/etc/kubeadm.yml"
...
def _get_instance_name():
    ...
    while True:
        try:
            ...
            click.echo(f"[ERROR] - CANNOT get the instance name: {e}")

        if time.time() - start > 1800:  # greater than 30 minutes
            raise Exception("CANNOT get the instance name")
...
@click.command("get-instance-name", help="Get the vServer name of the current instance")
@click.option("-s", "--short", default=False, is_flag=True, help="Short or long output")
def get_instance_name(short):
    try:
        ...
        click.echo(f"[ERROR] - Failed to get instance name: {e}")
        raise SystemExit(1)
...
@click.option("-p", "--path", default=_kubeadmin_config_path, is_flag=False, help="The path of the kubeadm config file")
def prepare_kubeadm_config(path):
    ...
    with open(path, "w") as stream:
        try:
            ...
            click.echo(
                f"[INFO] - Kubeadm config content is written to {path}")
...
cli.add_command(get_instance_name)  # noqa
